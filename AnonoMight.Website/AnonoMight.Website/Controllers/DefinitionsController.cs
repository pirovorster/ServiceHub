...
{
	public class DefinitionsController : BaseSiteController
	{
		...
		public ActionResult Content(string site, int page = 1)
		{
			using (AnonoMightEntities anonoMightEntities = ContextFactory.GetContext(site))
			{
				...
				if (currentSite != null)
				{
					return View(new DefinitionModel(currentSite, anonoMightEntities.Definitions, page));
				}
				else
		...
		{
			get { return new DefinitionsContextFactory(); }
		}
	}
}