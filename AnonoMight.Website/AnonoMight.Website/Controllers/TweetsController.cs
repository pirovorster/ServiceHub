...
{
	public class TweetsController : BaseSiteController
	{
		public ActionResult Content(string site, int page = 1)
		{
			using (AnonoMightEntities anonoMightEntities = ContextFactory.GetContext(site))
			{
				...
				if (currentSite != null)
				{
					return View(new TweetModel(currentSite, anonoMightEntities.Tweets, page));
				}
				else
		...
		{
			get { return new TweetsContextFactory(); }
		}
	}
}