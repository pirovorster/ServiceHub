	...
	public class AccountController : Controller
	{
		...
		public async Task<ActionResult> Register(RegisterViewModel model)
		{
			if (ModelState.IsValid)
			{
				...
				var user = new ApplicationUser() { UserName = model.UserName, IsConfirmed = false, ConfirmationToken = token };
				var result = await UserManager.CreateAsync(user, model.Password);
				if (result.Succeeded)
				{
					SendEmailConfirmation(model.UserName, token);
		...
		[HttpPost]
		[ValidateAntiForgeryToken]
		public ActionResult ChangeUsername(ChangeUsernameViewModel changeUsernameViewModel)
		{
			ViewBag.Message = string.Empty;
			if (ModelState.IsValid)
			{
				...
				using (ApplicationDbContext context = new ApplicationDbContext())
				{
					...
					ApplicationUser existingUser = context.Users.SingleOrDefault(u => u.UserName == changeUsernameViewModel.UserName);
					if (user != null && existingUser == null)
					{
						user.NewUsername = changeUsernameViewModel.UserName;
						user.ConfirmationToken = token;
						context.SaveChanges();
						SendChangeEmailConfirmation(changeUsernameViewModel.UserName, token);
						return RedirectToAction("WaitingForConfirmation");

					}
					else
						ViewBag.Message = "Could not change email. Contact site administrator.";
				}
		...
		[AllowAnonymous]
		public ActionResult ConfirmationReceived()
		{
			return View();
		}
		...
		{
			if (ConfirmAccountChange(token))
			{
				return RedirectToAction("ConfirmationReceived");

			}
			return RedirectToAction("WaitingForConfirmation");
		}

		private bool ConfirmAccountChange(string confirmationToken)
		{
			using (ApplicationDbContext context = new ApplicationDbContext())
			{
